    ...
    for i, f in enumerate(valid):
        with h5py.File(f, "r") as h:
            ...
            if os.path.exists(out_path) and os.path.getsize(out_path) > 1_000_000:
                print(f"  [{i+1}/{len(valid)}] skip existing episode_{i}.hdf5", flush=True)
                continue
            with h5py.File(out_path, "w") as out:
                out.attrs["sim"] = True
                obs = out.create_group("observations")
                # qpos = [joint_pos[:7] (7), call_button_lit (1)] -> 8-dim
                jp = d["obs/joint_pos"][sl][:, :7]
                if "obs/call_button_lit" in d:
                    led = d["obs/call_button_lit"][sl].reshape(-1, 1).astype(np.float32)
                    qpos = np.concatenate([jp, led], axis=1)
                else:
                    qpos = np.concatenate([jp, np.zeros((jp.shape[0], 1), dtype=np.float32)], axis=1)
                obs.create_dataset("qpos", data=pad(qpos))
                # qvel: pad with zero for LED channel
                jv = d["obs/joint_vel"][sl][:, :7]
                qvel = np.concatenate([jv, np.zeros((jv.shape[0], 1), dtype=np.float32)], axis=1)
                obs.create_dataset("qvel", data=pad(qvel))
                ...
                # action: pad to 8-dim (channel 8 = zero placeholder, ignored at rollout)
                # ACT's detr_vae shares state_dim for qpos input AND action output
